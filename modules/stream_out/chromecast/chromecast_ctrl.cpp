                                 ...
                                 const httpd_message_t *query )
{
    ...
    if( answer->i_body_offset == 0 )
    {
        ...
        std::string url = getSourceUrl();
        std::string mime = getSourceMime();
        if( url.empty() )
        {
        ...
        bool b_partial = chromecast_ParseByteRange( query, i_size, &i_start, &i_end );
